/*:
 * @target MZ
 * @plugindesc [v1.0.3] Ajusta posições de atores e inimigos (layouts: diagonal, horizontal, vertical, escada, grid) com presets, escala, snap e alinhamento. - AncientSouls
...
(() => {
    ...
    ActorLayout = ActorLayout.toLowerCase();
    EnemyLayout = EnemyLayout.toLowerCase();
    if (presetActive) {
        ActorBaseX = 1200;
        ActorBaseY = 540;
        ActorSpacingX = 160;
    ...
    function computeEnemyPositions() {
        ...
        const baseXRatio = 0.25; // 25% da tela
        const baseYRatio = 0.5;  // meio vertical
        const spacingXRatio = 140/1920;
        const spacingYRatio = 64/1080;
        let positions = [];
        if (EnemyLayout === 'grid') {
            let cols = EnemyGridCols > 1 ? EnemyGridCols : Math.ceil(Math.sqrt(count));
            const cellW = Math.floor(width * 0.20 / cols);
            const cellH = Math.floor(height * 0.25 / Math.ceil(count/cols));
            const originX = Math.round(width * baseXRatio);
            const originY = Math.round(height * 0.40);
            for (let i=0;i<count;i++) {
                const r = Math.floor(i/cols);
                const c = i % cols;
                let x = originX + c * cellW;
                let y = originY + r * cellH;
                positions.push({x,y});
            }
        } else {
            for (let i=0;i<count;i++) {
                ...
                switch(EnemyLayout){
                    ...
                    case 'vertical':
                        ...
                        break;
                    case 'escada':
                        // já diagonal, reforça mantendo
                        break;
                    case 'escadaInvertida':
                        y = Math.round(height * baseYRatio) - i * Math.round(height * spacingYRatio);
                        x = Math.round(width * (baseXRatio + i * spacingXRatio));
                        break;
                    case 'diagonal':
                    default:
                        break;
